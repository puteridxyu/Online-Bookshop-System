...
$option = mysqli_query($connection, "SELECT * FROM tracking");

if(count($_POST)>0)
{
    $id = $_POST['id'];
	mysqli_query($connection,"UPDATE tracking set tsta='" . $_POST['tsta'] . "' WHERE `tracking`.`id` = $id");
    $update = "Update has been successfully.!";
}

$edit = mysqli_query($connection,"SELECT * FROM tracking WHERE `tracking`.`id` = 1");
...
<section>
 ...
 <div class="container-fluid h-100 bg-light text-dark">
        ...
        <div>
            ...
            <a href="http://localhost/sd/Application%20Layer/Manage%20Delivery/adminorderview.php">Back to list</a>
            <h5 class="text-success text-center" id="update"><?=$update;?><h5>
                <form action="http://localhost/sd/Application%20Layer/Manage%20Delivery/editorderlist.php" method="POST">
                
                <br />
                <div class="form-group">
				    ID<input type="text" name="id" ><br><br>
                    STATUS<input type="text" class="form-control" name="tsta" value= "">  
                </div>
